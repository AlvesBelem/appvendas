...
class MainApp(App):
    ...
    def on_start(self):
        ...
        arquivos = os.listdir(f"icones/fotos_perfil")
    ...
    def carregar_infos_usuarios(self):
        try:
            ...
            # pegar informaçõies de usuário
            requisicao = requests.get(
                f"https://aplicativosvendashash-22f77-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}")
            ...
            pagina_ajustes = self.root.ids["homepage"]
            pagina_ajustes.ids["label_total_vendas"].text = f"[color=#000000]Total de Vendas:[/color] R$ [b]{total_vendas}[/b]"
    ...
    def mudar_foto_perfil(self, foto, *args):
        ...
        info = f'{{"avatar": "{foto}"}}'
        requisicao = requests.patch(f"https://aplicativosvendashash-22f77-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}",
                                    data=info)
        self.mudar_tela("ajustespage")

    def adicionar_vendedor(self, id_vendedor_adicionado):

        link = f'https://aplicativosvendashash-22f77-default-rtdb.firebaseio.com/.json?orderBy="id_vendedor"&equalTo="{id_vendedor_adicionado}"'
        ...
        if requisicao_dic == {}:
            ...
            if id_vendedor_adicionado in equipe:
                ...
                requests.patch(f"https://aplicativosvendashash-22f77-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}",
                               data=info)
    ...
    def selecionar_cliente(self, foto, *args):
        ...
        # pintar de azul a letra do item
    ...
    def adicionar_venda(self):
        ...
        if not quantidade:
            ...
            # dado que tudo foi preenchido vamos add a venda no banco e gerar o banner de vendas
        ...
        if cliente and produto and unidade and preco and quantidade and (type(preco) == float) and (type(quantidade) == float):
            ...
            info_venda = f'{{"cliente": "{cliente}", "produto": "{produto}", "foto_cliente": "{foto_cliente}", ' \
                    f'"foto_produto": "{foto_produto}", "data": "{data}", "unidade": "{unidade}", "preco": "{preco}", ' \
                    f'"quantidade": "{quantidade}"}}'

            requisicao_venda = requests.post(f"https://aplicativosvendashash-22f77-default-rtdb.firebaseio.com/{self.local_id}/vendas.json?auth={self.id_token}",
                              data=info_venda)
            ...
            pagina_ajustes = self.root.ids["homepage"]
            pagina_ajustes.ids[
                "label_total_vendas"].text = f"[color=#000000]Total de Vendas:[/color] R$ [b]{total_vendas}[/b]"
    ...
    def carregar_todas_vendas(self):
        ...
        # preenche a oagina todasvendaspage

        # pegar informaçõies da empresa
        requisicao = requests.get(
    ...
    def carregar_vendas_vendedor(self, dic_vendedor, *args):
        ...
        pagina_vendasoutrovendedorpage.ids[
            "label_total_vendas"].text = f"[color=#000000]Total de Vendas:[/color] R$ [b]{total_vendas}[/b]"
        # mudar foto perfil
